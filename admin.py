...
class HomeSummaryAdmin(ImportExportModelAdmin):
	change_list_template ='admin/truehome/home_summary_change_list.html'
	...
	def changelist_view(self, request,extra_context=None):
		...
		'count':summary_lastsoldprice.filter(lastsoldprice__gte=low+binsize*i,lastsoldprice__lte=low+binsize*(i+1)).count(),
		'pct':summary_lastsoldprice.filter(lastsoldprice__gte=low+binsize*i,lastsoldprice__lte=low+binsize*(i+1)).count(),
